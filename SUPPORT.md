...
- **Feature requests**: open a GitHub Issue describing the use case, not just the API
  shape you want — this helps us evaluate whether it fits the library's design
  philosophy (see [README.md](README.md#design-philosophy)).
- **Security vulnerabilities**: see [SECURITY.md](SECURITY.md) — do not use a public
  issue for these.

## Response times

LitheJ is maintained on a best-effort basis. There is no guaranteed response time or
SLA. Well-scoped pull requests with tests are generally the fastest way to get a fix
merged.
